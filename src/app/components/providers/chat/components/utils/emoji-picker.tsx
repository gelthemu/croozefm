...
const EmojiPicker = ({ onEmojiSelect }: EmojiPickerProps) => {
  ...
  return (
    <div className="w-full bg-light dark:bg-dark rounded-sm shadow shadow-gray/20 dark:shadow-light/10 p-2">
      <div className="flex flex-row items-center justify-between space-x-2">
        {emojis.map((emoji, index) => (
          <button
            key={index}
            className="h-8 w-8 flex items-center justify-center text-lg hover:bg-gray/10 dark:hover:bg-light/5 rounded-md cursor-pointer transition-colors focus:outline-none"
...
const AdminBadge = () => {
  return (
    <svg
      ...
      aria-label="Admin"
      role="img"
      className="inline-block w-[16px] h-[16px] max-w-[20px] mx-h-[20px] fill-current align-center select-none ml-0.5 mb-0.5"
...
const UserBadge = () => {
  return (
    <svg
      ...
      aria-label="User"
      role="img"
      className="inline-block w-[14px] h-[14px] max-w-[20px] mx-h-[20px] fill-current align-center select-none ml-0.5 mb-0.5"
      data-testid="icon-verified"
    >
      <g>
...
export { EmojiPicker, AdminBadge, UserBadge };
